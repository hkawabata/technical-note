...
$$\begin{cases} \displaystyle z_2 = \sum_i x^{\prime}_i \\ x_{N+i} = \begin{cases} \displaystyle b_i - \sum_{j=1}^N a_{ij} x_j, & \mbox{if } b_i \ge 0 \\ \displaystyle b_i - \sum_{j=1}^N a_{ij} x_j + x^{\prime}_i, & \mbox{if } b_i \lt 0 \end{cases} \\ x_1, ..., x_{N+2M} \ge 0 \end{cases}$$
