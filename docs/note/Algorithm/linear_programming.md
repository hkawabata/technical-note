...
→ 係数が大きい $$y$$ を選び、残りの非基底変数 $$x (= 0)$$ を固定し、**基底変数 $$s_1, s_2, s_3$$ の非負条件が破れない範囲で** 増やすことにする。

連立方程式 (A), (B), (C) において$$y$$ を0から少しずつ増やしていったとき、4を少し超えた $$y=4 + \delta (1 \gg \delta \gt 0)$$ で (A) の非負条件が破れる（$$s_1 \ge 0$$ が保てなくなる）：
