...
連立方程式 (A), (B), (C) において、方程式は3つ、変数は $$x, y, s_1, s_2, s_3$$ の5つなので、自由度は2。

- 非基底変数：(Z) に現れる $$x, y$$
- 基底変数：(Z) に現れない $$s_1, s_2, s_3$$

自由度2より非基底変数 $$(x, y) = (0, 0)$$ とおくことができて、このとき

$$(s_1, s_2, s_3) = (8, 7, 15)$$

$$z = x + 2y = 0$$

ここで、$$x, y$$ を増やしてもっと $$z$$ を大きくできないか考える。

$$z = x + 2y$$ と $$x, y \ge 0$$ より、係数が正である $$x, y$$ のどちらを増やしても $$z$$ が増加する。

→ 係数が大きい $$y$$ を **基底変数 $$s_1, s_2, s_3$$ の非負条件が破れない範囲で** 増やすことにする。

連立方程式 (A), (B), (C) において$$y$$ を0から少しずつ増やしていったとき、$$y=4$$ で (A) が非負条件の限界に達する（$$s_1 \ge 0$$ が保てなくなる）：

$$-x + 8 + s_1 = 8 \Leftrightarrow -x + s_1 = 0$$
