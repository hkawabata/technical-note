...
> **最小添字規則**：
> 
> 1. ステップ1において、係数 $$c_k \gt 0$$ な非基底変数 $$x_k$$（= ピボット列）を選ぶ際、候補の中で添え字 $$k$$ が最も小さい $$x_k$$ を選択する
> 2. ステップ2において、一番すぐに限界を迎える基底変数（= ピボット行）を選ぶ際、候補の中で添え字 $$l$$ が最も小さい $$x_l$$
> 
...
→ 係数の大きい $$y$$ を選び、**基底変数 $$s_1, s_2, s_3$$ の非負条件が破れない範囲で** 増やすことにする（最大係数規則）（追記：本当は巡回のリスクを回避するため最小添字規則を採用した方が安全）。
