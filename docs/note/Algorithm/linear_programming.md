...
>   - ただし、$$b_i \lt 0$$ の場合は $$x_{N+i}$$ が負になって解の条件を満たさないため、$$b_i$$ がゼロ以上になるように工夫してやる必要がある（次節）
...
- 最適値が負（$$max(z^{\prime}) \lt 0$$）の場合：**元の問題は実行不可能**

> **【NOTE】補助問題自体は最適解を持つのか？**
> 
> - $$b_i \ge 0$$ の方程式では元のスラック変数 $$x_{N+i}$$ を基底変数とみなして $$x_1, ..., x_N = 0$$
> - $$b_i \lt 0$$ のときは新しく追加したスラック変数 $$t_i$$ を基底変数とみなして $$x_1, ..., x_N = 0$$, $$x_{N+i} = 0$$
> 
> とすれば、基底変数についても非負条件を満たす「明らかな解」が得られるので、この補助問題は $$b_i \ge 0$$ の場合のシンプレックス法で解ける。
