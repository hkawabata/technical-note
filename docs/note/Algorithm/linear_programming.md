...
- $$b_i \ge 0$$ のときは $$x_{N+i} = 0$$
- $$b_i \lt 0$$ のときは $$x^{\prime}_i = 0$$

とおけば非負条件を満たす実現可能基底解が得られるので、この補助問題はシンプレックス法で解ける。

- 最適値がゼロ（$$max(z_2) = 0$$）の場合：**補助問題の最適解 = 元の問題の初期実行可能基底解として良い**
- 最適値が正（$$max(z_2) \ge 0$$）の場合：**元の問題は実行不可能**
...
2. 存在しなければ終了。**この時点の非基底変数をゼロと置いて得られる解が最適解**
	- 負の係数しか存在しない = どの非基底変数を増やしても目的関数は減少してしまう
3. 各基底変数が非負条件を破らない範囲の上限まで $$x_k$$ を増加させる
	- 際限なく増やしていくと、いずれかの基底変数が0を下回ってしまい実行可能領域外へ
	- 一番すぐに限界を迎える基底変数 $$x_l$$ を見つけることで、$$x_k$$ を増やせる上限がわかる
	- このとき、基底変数 $$x_l$$ は下限値（ゼロ）になっている
4. $$x_l$$ を非基底変数、$$x_k$$ を基底変数にし、問題を新たなスラック形に変形（役割の入れ替え）
5. 1に戻る
